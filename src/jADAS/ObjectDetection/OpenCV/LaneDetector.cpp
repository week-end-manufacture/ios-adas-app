...
Mat LaneDetector::detect_lane(Mat image) {
    ...
    //cout << "val1: " << lines[0] << endl;
    //cout << "val2: " << lines[1] << endl;
    //cout << "val3: " << lines[2] << endl;
    
    return draw_lines(image, lines);
}

vector<Point> LaneDetector::detect_point(Mat image) {
    Mat colorFilteredImage = filter_only_yellow_white(image);
    Mat regionOfInterest = crop_region_of_interest(colorFilteredImage);
    Mat edgesOnly = detect_edges(regionOfInterest);
    ...
    vector<Point> res;
    res.push_back(lane_data(image, lines, 0));
    res.push_back(lane_data(image, lines, 1));
    res.push_back(lane_data(image, lines, 3));
    res.push_back(lane_data(image, lines, 2));
...
Point LaneDetector::lane_data(Mat image, vector<Vec4i> lines, int select) {
    vector<double> rightSlope, leftSlope, rightIntercept, leftIntercept;
