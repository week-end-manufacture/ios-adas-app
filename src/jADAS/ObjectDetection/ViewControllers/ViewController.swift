...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
  ...
  override func viewWillDisappear(_ animated: Bool) {
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        if let videoPreviewLayerConnection = previewView.previewLayer.connection {
            let deviceOrientation = UIDevice.current.orientation
            guard let newVideoOrientation = AVCaptureVideoOrientation(deviceOrientation: deviceOrientation), deviceOrientation.isLandscape else {return}
...
//
//    let lanePoint = LaneDetectorBridge().detectLanePoint(image).map { $0.cgPointValue }
    //print(lanePoint)
    
//    let lanePath = UIBezierPath()
//    lanePath.lineWidth = 3.0
//    lanePath.move(to: lanePoint[0])
//    lanePath.addLine(to: lanePoint[1])
...
//    lanePath.stroke()
  ...
  @objc  func runModel(onPixelBuffer pixelBuffer: CVPixelBuffer) {
    ...
    DispatchQueue.main.async {
      ...
      self.drawAfterPerformingCalculations(onInferences: displayResult.inferences, withImageSize: CGSize(width: CGFloat(width), height: CGFloat(height)), lanePoint1: lanePoint[0], lanePoint2: lanePoint[1], lanePoint3: lanePoint[2], lanePoint4: lanePoint[3])
    }
  }
   ...
   */
    func drawAfterPerformingCalculations(onInferences inferences: [Inference], withImageSize imageSize:CGSize, lanePoint1:CGPoint, lanePoint2:CGPoint, lanePoint3:CGPoint, lanePoint4:CGPoint) {
    ...
    for inference in inferences {
      ...
      let size = string.size(usingFont: self.displayFont)
        ...
        let convertedLanePoint1 = CGPoint(x: lanePoint1.x / 2.5, y: lanePoint1.y / 2.5)
        let convertedLanePoint2 = CGPoint(x: lanePoint2.x / 2.5, y: lanePoint2.y / 2.5)
        let convertedLanePoint3 = CGPoint(x: lanePoint3.x / 2.5, y: lanePoint3.y / 2.5)
        let convertedLanePoint4 = CGPoint(x: lanePoint4.x / 2.5, y: lanePoint4.y / 2.5)
     
      let objectOverlay = ObjectOverlay(name: string, borderRect: convertedRect, nameStringSize: size, color: inference.displayColor, font: self.displayFont, lanePoint1: convertedLanePoint1, lanePoint2: convertedLanePoint2, lanePoint3: convertedLanePoint3, lanePoint4: convertedLanePoint4)
